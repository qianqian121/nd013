...
def get_model_crop_nvidia(time_len=1):
    ch, row, col = 3, 160, 320  # camera format
    ...
    model.add(Convolution2D(64, 3, 3))
    model.add(Flatten())
    model.add(Dense((11640), activation='relu'))
...
def get_model_comma_0305(time_len=1):
...
import cv2
batch_num = 0
def data_gen_opencv(imglist, batchsize):
    global batch_num
    imgpath = 'IMG'
    batch = 0
    num_samples = len(imglist)
    print(num_samples)
    while 1:
        random.shuffle(imglist)
        # print(imglist)
        for offset in range(0, num_samples, batchsize):
            # print(tup)
            batch_samples = imglist[offset : offset + batchsize]
            images = []
            angles = []
            for tup in batch_samples:
                # img = Image.open(imgpath + '/' + tup[0])
                img = cv2.imread(imgpath + '/' + tup[0])
...
def data_gen_faster_than_opencv(imglist, batchsize):
    global batch_num
    imgpath = 'IMG'
    batch = 0
    images = []
...
def data_gen_0305bak(imglist, batchsize):
    global batch_num
    imgpath = 'IMG'
    batch = 0
    while 1:
...
if __name__ == "__main__":
    ...
    history = model.fit_generator(
        ...
        samples_per_epoch = samples / batch,
